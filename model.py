...
def encoding_and_scaling(df: pd.DataFrame, app=False):
    ...
    categorical = ['Class']
    numeric_features = [col for col in df.columns if col not in categorical and col not in ['satisfaction', 'id']]
    ...
    if not app:
        ...
        column_transformer = pickle.load(open('data/column_transformer.pkl', 'rb'))
